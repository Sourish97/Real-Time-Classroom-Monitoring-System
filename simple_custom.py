...
def predict(x):
    ...
    x_rev /= 255
    p = np.zeros((1, 10))
    p[:,0:5] = model[0].predict(x.reshape(1,48,48,1))
    p[:,5:10] = model[1].predict(x_rev.reshape(1,48,48,1))
...
# ind = df[df['Name'] == 'sourish'].index.values.astype(int)
# print(ind[0])
# df.loc[ind,'t_focused'] = 5
# print(df)
...
# hn = [df['Name'][i] for i in range(len(df['Name']))]
# print(df['Name']['sourish'])
